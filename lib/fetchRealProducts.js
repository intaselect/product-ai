console.log("VERSION 6 EGYPT FALLBACK FIX");
...
export async function fetchRealProducts(query, country = "sa") {
  ...
  if (country === "eg") {
    ...
    // ✅ Fallback لو النتائج قليلة
    if (rawResults.length < 5 && apiKey) {
      const res = await fetch(
      ...
      const fallbackResults = (data.organic_results || []).map((item, i) => ({
        id: `fallback-${i + 1}`,
        title: item.title || "No title",
        price: item.snippet || "",
    ...
    return unique.slice(0, 60).map((item, i) => ({
      id: String(i + 1),
      title: item.title,
      price: item.price || 0,
      store: item.source,
