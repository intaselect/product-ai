...
if (country === "eg") {
  ...
  const fallbackUrl = `https://serpapi.com/search.json?engine=google&q=${encodeURIComponent(
    `${query} سعر مصر site:amazon.eg/dp OR site:noon.com/egypt-ar OR site:jumia.com.eg`
  )}&api_key=${apiKey}&gl=eg&hl=ar&google_domain=google.com.eg&num=60`;

  const fallbackData = await fetchJsonSafe(fallbackUrl, 8000);
  const organic = fallbackData?.organic_results || [];

  const results = organic
    .filter((item) => {
      const link = item.link || "";
      return (
        link.includes("amazon.eg") ||
        link.includes("noon.com") ||
        link.includes("jumia.com.eg")
      );
    })
    .map((item, i) => {
      ...
      return {
        ...
        title: item.title || "No title",
        price,
        priceText: price ? `${price.toLocaleString("en-US")} EGP` : "السعر داخل المتجر",
        store: item.source || item.displayed_link || "Egypt Store",
        image:
          item.thumbnail ||
          `https://placehold.co/400x400/ffffff/111111?text=Product`,
        url: item.link,
      };
    })
    .filter((item) => item.url && item.title);
