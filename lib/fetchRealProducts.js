...
// ✅ الكويت بنفس طريقة السعودية
if (country === "kw") {
  const res = await fetch(
    `https://serpapi.com/search.json?engine=google_shopping&q=${encodeURIComponent(
      query
    )}&api_key=${apiKey}&gl=kw&hl=ar&google_domain=google.com.kw&location=Kuwait`
  );
  ...
  return mapShoppingResults(data.shopping_results, "kw").map((item) => ({
    ...item,
    image:
      item.image ||
      `https://placehold.co/400x400?text=${encodeURIComponent(query)}`,
...
// ✅ قطر بنفس طريقة السعودية
if (country === "qa") {
  const res = await fetch(
    `https://serpapi.com/search.json?engine=google_shopping&q=${encodeURIComponent(
      query
    )}&api_key=${apiKey}&gl=qa&hl=ar&google_domain=google.com.qa&location=Qatar`
  );
  ...
  return mapShoppingResults(data.shopping_results, "qa").map((item) => ({
    ...item,
    image:
      item.image ||
      `https://placehold.co/400x400?text=${encodeURIComponent(query)}`,
...
// ✅ البحرين بنفس طريقة السعودية
if (country === "bh") {
  const res = await fetch(
    `https://serpapi.com/search.json?engine=google_shopping&q=${encodeURIComponent(
      query
    )}&api_key=${apiKey}&gl=bh&hl=ar&google_domain=google.com.bh&location=Bahrain`
  );
  ...
  return mapShoppingResults(data.shopping_results, "bh").map((item) => ({
    ...item,
    image:
      item.image ||
      `https://placehold.co/400x400?text=${encodeURIComponent(query)}`,
