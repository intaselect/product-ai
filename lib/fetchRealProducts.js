...
export async function fetchRealProducts(query, country = "sa", ip = null)  {
 ...
 if (cleanCountry === "eg") {
  console.log("🇪🇬 Egypt: scrapers first, then limited SerpAPI fallback");

  const scraperResults = (
    await Promise.all([
      scrapeAmazonEgypt(cleanQuery),
      scrapeJumiaEgypt(cleanQuery),
      scrapeNoonEgypt(cleanQuery),
    ])
  )
    .flat()
  ...
  if (scraperResults.length < 8 && (!shopping || shopping.length === 0)) {
    console.log("⚠️ Egypt scrapers low + shopping empty → one organic fallback");
    ...
    )}&api_key=${apiKey}&gl=eg&hl=ar&google_domain=google.com.eg&num=10`;
  ...
  const results = [...scraperResults, ...(shopping || []), ...organicResults]
    .filter((item) => item.url && item.title)
    .slice(0, 40);
