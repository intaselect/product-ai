...
export async function fetchRealProducts(
  ...
  if (!allowSerpApi) {
  console.log("🚫 SERPAPI BLOCKED: fetchRealProducts called without allowSerpApi");
  return [];
}
  ...
  // ✅ بيانات إضافية من نفس نتيجة SerpAPI بدون سحب جديد
  description:
    item.snippet ||
    item.description ||
    item.product_description ||
    "",
  features: Array.isArray(item.extensions) ? item.extensions : [],
  specifications: item.product_result || {},
  source_brand:
    item.brand ||
    item.product_brand ||
    "",
 ...
 if (cleanCountry === "eg") {
  console.log("🇪🇬 Egypt: hybrid google shopping + organic fallback");

  const shopping = await fetchGccShopping(cleanQuery, "eg");

  const goodShopping = (shopping || []).filter(
    (item) =>
      item.title &&
      item.url &&
      item.image &&
      !String(item.image).includes("placehold.co")
  );

  const searchUrl = `https://serpapi.com/search.json?engine=google&q=${encodeURIComponent(
    `${cleanQuery} سعر مصر site:amazon.eg OR site:noon.com OR site:jumia.com.eg`
  ...
  const results = [...goodShopping, ...organicResults]
    .filter((item) => item.url && item.title)
