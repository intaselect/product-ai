...
export async function fetchRealProducts(query, country = "sa", ip = null)  {
  ...
  async function fetchGccShopping(query, country) {
    const config = {
      ...
      iq: { gl: "iq", google_domain: "google.com.iq" },
      om: { gl: "om", google_domain: "google.com.om" },
    };
...
// ✅ العراق
if (cleanCountry === "iq") {
  const results = await fetchGccShopping(cleanQuery, "iq");
  await saveResultsToCache(results);
  return results;
}
...
// ✅ عمان
if (cleanCountry === "om") {
  const results = await fetchGccShopping(cleanQuery, "om");
  await saveResultsToCache(results);
  return results;
}
  if (cleanCountry === "eg") {
