...
export async function fetchRealProducts(query, country = "sa") {
  ...
  country,
  query,
  hasSerpApiKey: !!apiKey,
  keyLength: apiKey ? apiKey.length : 0,
});
if (!apiKey) {
  console.log("❌ SERPAPI_KEY is missing on server");
  return [];
}
...
if (country === "eg") {
  ...
  // 1️⃣ نحاول Google Shopping الأول
  const shopping = await fetchGccShopping(query, "eg");

  if (shopping && shopping.length > 0) {
    return shopping;
  }

  console.log("⚠️ Egypt Shopping empty → Google Search + Google Images fallback");

  // 2️⃣ Google العادي للروابط
  const searchUrl = `https://serpapi.com/search.json?engine=google&q=${encodeURIComponent(
    `${query} سعر مصر site:amazon.eg OR site:noon.com OR site:jumia.com.eg`
  )}&api_key=${apiKey}&gl=eg&hl=ar&google_domain=google.com.eg&num=80`;

  // 3️⃣ Google Images للصور
  const imagesUrl = `https://serpapi.com/search.json?engine=google_images&q=${encodeURIComponent(
    `${query} product`
  )}&api_key=${apiKey}&gl=eg&hl=ar&google_domain=google.com.eg`;

  const [searchData, imageData] = await Promise.all([
    fetchJsonSafe(searchUrl, 8000),
    fetchJsonSafe(imagesUrl, 8000),
  ]);
const searchQueries = [
  `${query} سعر مصر شراء اونلاين`,
  `${query} amazon.eg`,
  `${query} noon egypt jumia egypt`,
];
  ...
  searchQueries.map((q) =>
    fetchJsonSafe(
      `https://serpapi.com/search.json?engine=google&q=${encodeURIComponent(
        q
      )}&api_key=${apiKey}&gl=eg&hl=ar&google_domain=google.com.eg&num=30`,
      8000
    )
  )
);

const organic = searchResponses.flatMap((data) => data?.organic_results || []);
  const images = imageData?.images_results || [];
...
const image =
  images[i]?.thumbnail ||
  images[i + 1]?.thumbnail ||
  images[i + 2]?.thumbnail ||
  images[i]?.original ||
  images[i + 1]?.original ||
  "";
      ...
      return {
        ...
        image:
          image ||
          `https://placehold.co/400x400/ffffff/111111?text=${encodeURIComponent(
            query
          )}`,
        url: item.link,
      };
