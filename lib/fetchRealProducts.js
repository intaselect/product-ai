...
if (country === "eg") {
  ...
  const shoppingUrl = `https://serpapi.com/search.json?engine=google_shopping&q=${encodeURIComponent(
    query
  )}&api_key=${apiKey}&gl=eg&hl=ar`;

  const shoppingData = await fetchJsonSafe(shoppingUrl, 8000);

  let results = shoppingData?.shopping_results || [];

  // 2️⃣ لو مفيش نتائج → fallback على Google العادي
  if (!results || results.length === 0) {
    console.log("⚠️ Shopping فاضي → بنستخدم Google fallback");
    ...
    results = organic.map((item, i) => ({
      ...
      price:
        ...
        "",
      source: item.source || "Google Egypt",
      thumbnail: item.thumbnail || "",
      link: item.link,
    }));
  }

  return mapShoppingResults(results, "eg")
    .filter((item) => item.url && item.title)
    .map((item) => ({
      ...item,
      image:
