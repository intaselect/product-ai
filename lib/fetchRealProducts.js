export async function fetchRealProducts(query, country = "sa") {
  ...
  // 👇 دي الإضافة المهمة لمصر
  const egyptShoppingParams =
    country === "eg" ? "&tbs=mr:1,local_avail:1" : "";

  const res = await fetch(
    `https://serpapi.com/search.json?engine=google_shopping&q=${encodeURIComponent(
      query
    )}&api_key=${apiKey}&gl=${selectedCountry.gl}&hl=ar&google_domain=${selectedCountry.googleDomain}${locationParam}${egyptShoppingParams}`
  );
