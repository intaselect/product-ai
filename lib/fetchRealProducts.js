...
if (country === "eg") {
  ...
  console.log("⚠️ مفيش Shopping → fallback نظيف");

  // 2️⃣ fallback Google (منتجات فقط)
  const res = await fetch(
    `https://serpapi.com/search.json?engine=google&q=${encodeURIComponent(
      query + " سعر site:noon.com OR site:amazon.eg OR site:jumia.com.eg"
