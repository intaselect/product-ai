...
if (cleanCountry === "eg") {
  console.log("🇪🇬 Egypt: Google results first + scrapers with real images only");

  const shopping = await fetchGccShopping(cleanQuery, "eg");

  console.log("⚠️ Egypt → single Google Search fallback");
  ...
  try {
    ...
    scraperResults = [...amazon, ...jumia, ...noon]
    ...
    console.log("EG scrapers failed:", err?.message || err);
  }
